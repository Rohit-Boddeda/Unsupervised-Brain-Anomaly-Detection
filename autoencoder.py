...
class Autoencoder(nn.Module):
    ...
    def init_from_ckpt(self, path, ignore_keys=list()):
        print(f"==>> loading autoencoder weights from path: {path}", flush=True)
        sd = torch.load(path, map_location="cpu")["autoencoder"]
        keys = list(sd.keys())
        for k in keys:
        ...
        print(f"==>> loaded autoencoder weights complete", flush=True)
