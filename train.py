...
from i2sb.latent_runner import LatentRunner
...
def create_training_options():
    ...
    parser.add_argument(
        ...
        type=str,
        default='latent',
        help="Choose between latent and pixel model",
    )
...
def main(opt):
    ...
    train_dataset = dataset.IXIDataset(root_dir="/home/lalith/Latent_SB/Preprocessed_Data/IXI_jpg/",
                                        anomaly_source_path="/home/lalith/Latent_SB/dtd/images/",
                                        resize_shape=[256, 256])

    val_dataset = dataset.IXIDataset(root_dir="/home/lalith/Latent_SB/Preprocessed_Dataa/IXI_jpg/",
                                        anomaly_source_path="/home/lalith/Latent_SB/dtd/images/",
                                        resize_shape=[256, 256])
...
if __name__ == '__main__':
    ...
    if opt.distributed:
        ...
        for rank in range(size):
            ...
            print(f"==>> distributed opt.global_rank: {opt.global_rank}", flush=True)
            print(f"==>> distributed opt.global_size: {opt.global_size}", flush=True)
            print('Node rank %d, local proc %d, global proc %d, global_size %d' % (opt.node_rank, rank, global_rank, global_size), flush=True)
        ...
        opt.local_rank = 0
        opt.global_size = 1
        print(f"==>> not distributed opt.global_rank: {opt.global_rank}", flush=True)
        print(f"==>> not distributed opt.global_size: {opt.global_size}", flush=True)
